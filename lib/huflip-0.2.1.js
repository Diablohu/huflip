...
function huFlip(obj, el, settings){
	...
	this.obj
		...
		.css(_huFlip.cssprefix('transition'), _huFlip.cssprefix('transform')+' '+(huFlipObj.durationA/1000)+'s linear')
		.on({
			click:	function(){
...
huFlip.prototype.flip = function(huFlipObj){
	...
	// 开始元素动画
	o.css(_huFlip.cssprefix('transform'), huFlipObj.returnTransform('obj', 'end'));
	...
	setTimeout(function(){
		// 开始元素样式还原
		o.css(_huFlip.cssprefix('transform'), huFlipObj.returnTransform('obj', 'start'))
...
huFlip.prototype.initGlobal = function(){
	...
	// Style Sheet
	function addStyleSheet(selector, declaration, sheet){
		var v = ''
